...
async def save_uploaded_image(image_file, user_id, file_name):
    try:
        ...
        if token_usage > CONFIG.max_image_tokens:
            logger.error(
                msg=f"The given image has crossed the token limit. EPSID:{user_id}->ImageToken:{token_usage}"
            )
            return {
                "status": "error",
        ...
        file_id = await save_image_details(
            content, user_id, file_name, url, mime_type, token_usage
        )
...
async def generate_prompt_contents(
    ...
    contents = (
        [*previous_history, *question_part] if previous_history else [*question_part]
    )
...
async def generate_image_history_summary(user_history: CurrentConversation = None):
    ...
    for message in reversed(user_history.messages):
        previous_history.append(
            types.Content(
                role="user", parts=[types.Part.from_text(text=message.question)]
            )
        )
        if (
            message.image_details
            and message.image_details.image_url
            and message.image_details.image_mime_type
        ):
                ...
                types.Content(
                    ...
                    parts=[
                        types.Part.from_bytes(
                            data=image_bytes,
                            mime_type=message.image_details.image_mime_type,
                        )
                    ],
                )
            )
        previous_history.append(
            types.Content(
                role="model", parts=[types.Part.from_text(text=message.answer)]
            )
        )
